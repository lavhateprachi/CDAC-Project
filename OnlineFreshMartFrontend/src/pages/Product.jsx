...
// Product component
const Product = () => {
  const { productId, categoryId } = useParams(); // Get parameters from the URL

  let user = JSON.parse(sessionStorage.getItem("active-user")); // Get user data from session storage

  const [quantity, setQuantity] = useState(""); // State for storing the quantity
  ...
  useEffect(() => {
    ...
    // Fetch the product and products by category when the component mounts
  ...
  }, [productId]);// Run the effect only when the productId changes

  // Function to retrieve products by category from the server
  ...
  // Function to save the selected product to the cart
  ...
  const addToCart = (e) => {
    if (user == null) {
      ...
      saveProductToCart(user.id); // Save product to cart for logged-in user
    ...
    <div className="container-fluid">
      <div class="row">
        <div class="col-sm-2 mt-2">
          {/* Render the component to display all categories */}
          <GetAllCategories />
        </div>
              ...
              <div className="d-flex justify-content-between">
                <div>
                  <form class="row g-3" onSubmit={addToCart}>
                    ...
                    {/* Submit button and ToastContainer for notifications */}
                    <div class="col-auto">
                      <input
                        type="submit"
